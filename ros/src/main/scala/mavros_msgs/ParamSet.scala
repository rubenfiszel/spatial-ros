...
trait ParamSetApi extends ParamSetExp { self: RosApi =>

}

trait ParamSetExp { self: RosExp =>
  ...
  case class ParamSetRep(s: Exp[ParamSetRep]) extends MetaAny[ParamSetRep] {
    ...
    @api def value: ParamValue =
      ParamValue(stage(ParamSetRep_reply_value(s))(ctx))
  ...
  case class ParamSetRep_reply_success(srv: Exp[ParamSetRep])
      extends Op[Bool] {
    def mirror(f: Tx) = stage(ParamSetRep_reply_success(f(srv)))(EmptyContext)
  }

  case class ParamSetRep_reply_value(srv: Exp[ParamSetRep])
  ...
  case class NewParamSet(param_id: Exp[Text], value: Exp[ParamValue])
      extends Op[ParamSet] {
  ...
  object ParamSet {
    ...
    @api def apply(param_id: Text, value: ParamValue): ParamSet =
      ParamSet(stage(NewParamSet(param_id.s, value.s))(ctx))
  }
