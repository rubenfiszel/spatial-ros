...
trait GlobalPositionTargetExp {
  ...
  implicit object GlobalPositionTargetMsg extends Msg[GlobalPositionTarget]
...
trait ScalaGenGlobalPositionTarget extends RosScalaCodegen {
  val IR: RosExp
  import IR._

  override protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    case GlobalPositionTarget_coordinate_frame(msg) => emit(src"val $lhs = $msg.coordinate_frame")
    case GlobalPositionTarget_type_mask(msg) => emit(src"val $lhs = $msg.type_mask")
    case GlobalPositionTarget_latitude(msg) => emit(src"val $lhs = $msg.latitude")
    case GlobalPositionTarget_longitude(msg) => emit(src"val $lhs = $msg.longitude")
    case GlobalPositionTarget_velocity(msg) => emit(src"val $lhs = $msg.velocity")
    case GlobalPositionTarget_acceleration_or_force(msg) => emit(src"val $lhs = $msg.acceleration_or_force")
    case GlobalPositionTarget_yaw(msg) => emit(src"val $lhs = $msg.yaw")
    case GlobalPositionTarget_yaw_rate(msg) => emit(src"val $lhs = $msg.yaw_rate")
    case NewGlobalPositionTarget(coordinate_frame, type_mask, latitude, longitude, velocity, acceleration_or_force, yaw, yaw_rate) =>
      emit(src"val $lhs = GlobalPositionTarget($coordinate_frame, $type_mask, $latitude, $longitude, $velocity, $acceleration_or_force, $yaw, $yaw_rate)")
