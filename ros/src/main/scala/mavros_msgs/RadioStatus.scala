...
trait RadioStatusExp {
  ...
  implicit object RadioStatusMsg extends Msg[RadioStatus]
...
trait ScalaGenRadioStatus extends RosScalaCodegen {
  val IR: RosExp
  import IR._

  override protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    case RadioStatus_rssi(msg) => emit(src"val $lhs = $msg.rssi")
    case RadioStatus_remrssi(msg) => emit(src"val $lhs = $msg.remrssi")
    case RadioStatus_txbuf(msg) => emit(src"val $lhs = $msg.txbuf")
    case RadioStatus_noise(msg) => emit(src"val $lhs = $msg.noise")
    case RadioStatus_remnoise(msg) => emit(src"val $lhs = $msg.remnoise")
    case RadioStatus_rxerrors(msg) => emit(src"val $lhs = $msg.rxerrors")
    case RadioStatus_fixed(msg) => emit(src"val $lhs = $msg.fixed")
    case RadioStatus_rssi_dbm(msg) => emit(src"val $lhs = $msg.rssi_dbm")
    case RadioStatus_remrssi_dbm(msg) => emit(src"val $lhs = $msg.remrssi_dbm")
    case NewRadioStatus(rssi, remrssi, txbuf, noise, remnoise, rxerrors, fixed, rssi_dbm, remrssi_dbm) =>
      emit(src"val $lhs = RadioStatus($rssi, $remrssi, $txbuf, $noise, $remnoise, $rxerrors, $fixed, $rssi_dbm, $remrssi_dbm)")
