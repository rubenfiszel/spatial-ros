...
trait FileRenameApi extends FileRenameExp { self: RosApi =>

}

trait FileRenameExp { self: RosExp =>
  ...
  case class FileRename(s: Exp[FileRename]) extends MetaAny[FileRename] {
    ...
    @api def response: FileRenameRep =
      FileRenameRep(stage(FileRenameReply(s))(ctx))
    @api def ===(that: FileRename): Bool = ???
    @api def =!=(that: FileRename): Bool = ???
    @api def toText: Text                = textify(this)
  }

  case class FileRenameRep(s: Exp[FileRenameRep])
      extends MetaAny[FileRenameRep] {
    @api def success: Bool = Bool(stage(FileRenameRep_reply_success(s))(ctx))
    @api def r_errno: FixPt[TRUE, _32, _0] =
      FixPt(stage(FileRenameRep_reply_r_errno(s))(ctx))
  ...
  case class FileRenameRep_reply_success(srv: Exp[FileRenameRep])
      extends Op[Bool] {
    def mirror(f: Tx) =
      stage(FileRenameRep_reply_success(f(srv)))(EmptyContext)
  }

  case class FileRenameRep_reply_r_errno(srv: Exp[FileRenameRep])
      extends Op[FixPt[TRUE, _32, _0]] {
    def mirror(f: Tx) =
      stage(FileRenameRep_reply_r_errno(f(srv)))(EmptyContext)
  }
  ...
  case class NewFileRename(old_path: Exp[Text], new_path: Exp[Text])
      extends Op[FileRename] {
    def mirror(f: Tx) =
      stage(NewFileRename(f(old_path), f(new_path)))(EmptyContext)
  }

  object FileRename {

    @api def apply(old_path: Text, new_path: Text): FileRename =
      FileRename(stage(NewFileRename(old_path.s, new_path.s))(ctx))
  }
