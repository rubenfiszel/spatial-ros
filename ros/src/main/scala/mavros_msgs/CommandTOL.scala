...
trait CommandTOLApi extends CommandTOLExp { self: RosApi =>

}

trait CommandTOLExp { self: RosExp =>
  ...
  case class CommandTOL(s: Exp[CommandTOL]) extends MetaAny[CommandTOL] {
    ...
    @api def latitude: FltPt[_24, _8] =
      FltPt(stage(CommandTOL_latitude(s))(ctx))
    @api def longitude: FltPt[_24, _8] =
      FltPt(stage(CommandTOL_longitude(s))(ctx))
    @api def altitude: FltPt[_24, _8] =
      FltPt(stage(CommandTOL_altitude(s))(ctx))
    @api def response: CommandTOLRep =
      CommandTOLRep(stage(CommandTOLReply(s))(ctx))
    @api def ===(that: CommandTOL): Bool = ???
    @api def =!=(that: CommandTOL): Bool = ???
    @api def toText: Text                = textify(this)
  }

  case class CommandTOLRep(s: Exp[CommandTOLRep])
      extends MetaAny[CommandTOLRep] {
    @api def success: Bool = Bool(stage(CommandTOLRep_reply_success(s))(ctx))
    @api def result: FixPt[FALSE, _8, _0] =
      FixPt(stage(CommandTOLRep_reply_result(s))(ctx))
  ...
  case class CommandTOL_latitude(srv: Exp[CommandTOL])
      extends Op[FltPt[_24, _8]] {
    def mirror(f: Tx) = stage(CommandTOL_latitude(f(srv)))(EmptyContext)
  }

  case class CommandTOL_longitude(srv: Exp[CommandTOL])
      extends Op[FltPt[_24, _8]] {
    def mirror(f: Tx) = stage(CommandTOL_longitude(f(srv)))(EmptyContext)
  }

  case class CommandTOL_altitude(srv: Exp[CommandTOL])
      extends Op[FltPt[_24, _8]] {
    def mirror(f: Tx) = stage(CommandTOL_altitude(f(srv)))(EmptyContext)
  }

  case class CommandTOLRep_reply_success(srv: Exp[CommandTOLRep])
      extends Op[Bool] {
    def mirror(f: Tx) =
      stage(CommandTOLRep_reply_success(f(srv)))(EmptyContext)
  }

  case class CommandTOLRep_reply_result(srv: Exp[CommandTOLRep])
  ...
  case class NewCommandTOL(yaw: Exp[FltPt[_24, _8]],
                           latitude: Exp[FltPt[_24, _8]],
                           longitude: Exp[FltPt[_24, _8]],
                           altitude: Exp[FltPt[_24, _8]])
      extends Op[CommandTOL] {
    def mirror(f: Tx) =
      stage(NewCommandTOL(f(yaw), f(latitude), f(longitude), f(altitude)))(
        EmptyContext)
  }
  ...
  object CommandTOL {
    ...
    @api
    def apply(yaw: FltPt[_24, _8],
              latitude: FltPt[_24, _8],
              longitude: FltPt[_24, _8],
              altitude: FltPt[_24, _8]): CommandTOL =
      CommandTOL(
        stage(NewCommandTOL(yaw.s, latitude.s, longitude.s, altitude.s))(ctx))
  }
