...
trait FileCloseApi extends FileCloseExp { self: RosApi =>

}

trait FileCloseExp { self: RosExp =>
  ...
  case class FileClose(s: Exp[FileClose]) extends MetaAny[FileClose] {
    ...
    @api def response: FileCloseRep =
      FileCloseRep(stage(FileCloseReply(s))(ctx))
  ...
  case class FileCloseRep(s: Exp[FileCloseRep]) extends MetaAny[FileCloseRep] {
    ...
    @api def r_errno: FixPt[TRUE, _32, _0] =
      FixPt(stage(FileCloseRep_reply_r_errno(s))(ctx))
  ...
  case class FileCloseRep_reply_success(srv: Exp[FileCloseRep])
      extends Op[Bool] {
    def mirror(f: Tx) = stage(FileCloseRep_reply_success(f(srv)))(EmptyContext)
  }

  case class FileCloseRep_reply_r_errno(srv: Exp[FileCloseRep])
  ...
  object FileClose {
    ...
    @api def apply(file_path: Text): FileClose =
      FileClose(stage(NewFileClose(file_path.s))(ctx))
  }
