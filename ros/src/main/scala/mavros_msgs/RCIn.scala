...
trait RCInApi extends RCInExp { self: RosApi =>

}

trait RCInExp { self: RosExp =>
  ...
  case class RCIn(s: Exp[RCIn]) extends MetaAny[RCIn] {
    ...
    @api def channels: MetaArray[FixPt[FALSE, _16, _0]] =
      MetaArray(stage(RCIn_channels(s))(ctx))
    @api def ===(that: RCIn) = ???
    @api def =!=(that: RCIn) = ???
  ...
  case class RCIn_channels(msg: Exp[RCIn])
      extends Op[MetaArray[FixPt[FALSE, _16, _0]]] {
    def mirror(f: Tx) = stage(RCIn_channels(f(msg)))(EmptyContext)
  }

  case class NewRCIn(rssi: Exp[FixPt[FALSE, _8, _0]],
                     channels: Exp[MetaArray[FixPt[FALSE, _16, _0]]])
      extends Op[RCIn] {
  ...
  object RCIn {
    ...
    def apply(rssi: FixPt[FALSE, _8, _0],
              channels: MetaArray[FixPt[FALSE, _16, _0]]): RCIn =
      RCIn(stage(NewRCIn(rssi.s, channels.s))(ctx))
