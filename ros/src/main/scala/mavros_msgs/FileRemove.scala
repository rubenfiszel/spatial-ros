...
trait FileRemoveApi extends FileRemoveExp { self: RosApi =>

}

trait FileRemoveExp { self: RosExp =>
  ...
  case class FileRemove(s: Exp[FileRemove]) extends MetaAny[FileRemove] {
    ...
    @api def response: FileRemoveRep =
      FileRemoveRep(stage(FileRemoveReply(s))(ctx))
    @api def ===(that: FileRemove): Bool = ???
    @api def =!=(that: FileRemove): Bool = ???
    @api def toText: Text                = textify(this)
  }

  case class FileRemoveRep(s: Exp[FileRemoveRep])
      extends MetaAny[FileRemoveRep] {
    @api def success: Bool = Bool(stage(FileRemoveRep_reply_success(s))(ctx))
    @api def r_errno: FixPt[TRUE, _32, _0] =
      FixPt(stage(FileRemoveRep_reply_r_errno(s))(ctx))
  ...
  case class FileRemoveRep_reply_success(srv: Exp[FileRemoveRep])
      extends Op[Bool] {
    def mirror(f: Tx) =
      stage(FileRemoveRep_reply_success(f(srv)))(EmptyContext)
  }

  case class FileRemoveRep_reply_r_errno(srv: Exp[FileRemoveRep])
      extends Op[FixPt[TRUE, _32, _0]] {
    def mirror(f: Tx) =
      stage(FileRemoveRep_reply_r_errno(f(srv)))(EmptyContext)
  }
  ...
  object FileRemove {
    ...
    @api def apply(file_path: Text): FileRemove =
      FileRemove(stage(NewFileRemove(file_path.s))(ctx))
  }
