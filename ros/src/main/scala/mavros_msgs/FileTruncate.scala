...
trait FileTruncateExp {
  ...
  case class FileTruncateRep(s: Exp[FileTruncateRep]) extends MetaAny[FileTruncateRep] {
    ...
    @api def r_errno: FixPt[TRUE,_32,_0] = FixPt(stage(FileTruncateRep_reply_r_errno(s))(ctx))
...
case class FileTruncateRep_reply_success(srv: Exp[FileTruncateRep]) extends Op[Bool] {
  def mirror(f: Tx) = stage(FileTruncateRep_reply_success(f(srv)))(EmptyContext)
}

  
case class FileTruncateRep_reply_r_errno(srv: Exp[FileTruncateRep]) extends Op[FixPt[TRUE,_32,_0]] {
  def mirror(f: Tx) = stage(FileTruncateRep_reply_r_errno(f(srv)))(EmptyContext)
}
