...
trait ParamValueApi extends ParamValueExp { self: RosApi =>

}

trait ParamValueExp { self: RosExp =>
  ...
  case class ParamValue(s: Exp[ParamValue]) extends MetaAny[ParamValue] {
    @api def integer: FixPt[TRUE, _64, _0] =
      FixPt(stage(ParamValue_integer(s))(ctx))
  ...
  case class ParamValue_integer(msg: Exp[ParamValue])
      extends Op[FixPt[TRUE, _64, _0]] {
    def mirror(f: Tx) = stage(ParamValue_integer(f(msg)))(EmptyContext)
  }

  case class ParamValue_real(msg: Exp[ParamValue])
      extends Op[FltPt[_53, _11]] {
    def mirror(f: Tx) = stage(ParamValue_real(f(msg)))(EmptyContext)
  }

  case class NewParamValue(integer: Exp[FixPt[TRUE, _64, _0]],
                           real: Exp[FltPt[_53, _11]])
  ...
  object ParamValue {
    ...
    @api
    def apply(integer: FixPt[TRUE, _64, _0],
              real: FltPt[_53, _11]): ParamValue =
      ParamValue(stage(NewParamValue(integer.s, real.s))(ctx))
