...
trait ManualControlApi extends ManualControlExp { self: RosApi =>

}

trait ManualControlExp { self: RosExp =>
  ...
  case class ManualControl(s: Exp[ManualControl])
      extends MetaAny[ManualControl] {
    ...
    @api def buttons: FixPt[FALSE, _16, _0] =
      FixPt(stage(ManualControl_buttons(s))(ctx))
    @api def ===(that: ManualControl) = ???
    @api def =!=(that: ManualControl) = ???
    @api def toText: Text             = textify(this)
  }

  case class ManualControl_x(msg: Exp[ManualControl])
      extends Op[FltPt[_24, _8]] {
    def mirror(f: Tx) = stage(ManualControl_x(f(msg)))(EmptyContext)
  }

  case class ManualControl_y(msg: Exp[ManualControl])
      extends Op[FltPt[_24, _8]] {
    def mirror(f: Tx) = stage(ManualControl_y(f(msg)))(EmptyContext)
  }

  case class ManualControl_z(msg: Exp[ManualControl])
      extends Op[FltPt[_24, _8]] {
    def mirror(f: Tx) = stage(ManualControl_z(f(msg)))(EmptyContext)
  }

  case class ManualControl_r(msg: Exp[ManualControl])
      extends Op[FltPt[_24, _8]] {
    def mirror(f: Tx) = stage(ManualControl_r(f(msg)))(EmptyContext)
  }

  case class ManualControl_buttons(msg: Exp[ManualControl])
      extends Op[FixPt[FALSE, _16, _0]] {
    def mirror(f: Tx) = stage(ManualControl_buttons(f(msg)))(EmptyContext)
  }

  case class NewManualControl(x: Exp[FltPt[_24, _8]],
                              y: Exp[FltPt[_24, _8]],
                              z: Exp[FltPt[_24, _8]],
                              r: Exp[FltPt[_24, _8]],
                              buttons: Exp[FixPt[FALSE, _16, _0]])
      extends Op[ManualControl] {
    def mirror(f: Tx) =
      stage(NewManualControl(f(x), f(y), f(z), f(r), f(buttons)))(EmptyContext)
  }

  object ManualControl {

    @api
    def apply(x: FltPt[_24, _8],
              y: FltPt[_24, _8],
              z: FltPt[_24, _8],
              r: FltPt[_24, _8],
              buttons: FixPt[FALSE, _16, _0]): ManualControl =
      ManualControl(
        stage(NewManualControl(x.s, y.s, z.s, r.s, buttons.s))(ctx))
