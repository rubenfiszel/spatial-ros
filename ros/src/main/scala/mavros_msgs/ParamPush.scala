...
trait ParamPushExp {
  ...
  case class ParamPushRep(s: Exp[ParamPushRep]) extends MetaAny[ParamPushRep] {
    ...
    @api def param_transfered: FixPt[FALSE,_32,_0] = FixPt(stage(ParamPushRep_reply_param_transfered(s))(ctx))
...
case class ParamPushRep_reply_success(srv: Exp[ParamPushRep]) extends Op[Bool] {
  def mirror(f: Tx) = stage(ParamPushRep_reply_success(f(srv)))(EmptyContext)
}

  
case class ParamPushRep_reply_param_transfered(srv: Exp[ParamPushRep]) extends Op[FixPt[FALSE,_32,_0]] {
  def mirror(f: Tx) = stage(ParamPushRep_reply_param_transfered(f(srv)))(EmptyContext)
}
