...
trait FileOpenApi extends FileOpenExp { self: RosApi =>

}

trait FileOpenExp { self: RosExp =>
  ...
  case class NewFileOpen(file_path: Exp[Text], mode: Exp[FixPt[FALSE, _8, _0]])
      extends Op[FileOpen] {
  ...
  object FileOpen {
    ...
    @api def apply(file_path: Text, mode: FixPt[FALSE, _8, _0]): FileOpen =
      FileOpen(stage(NewFileOpen(file_path.s, mode.s))(ctx))
  }
