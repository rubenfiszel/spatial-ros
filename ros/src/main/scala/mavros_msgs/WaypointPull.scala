...
trait WaypointPullApi extends WaypointPullExp { self: RosApi =>

}

trait WaypointPullExp { self: RosExp =>
  ...
  case class WaypointPull(s: Exp[WaypointPull]) extends MetaAny[WaypointPull] {
    ...
    @api def response: WaypointPullRep =
      WaypointPullRep(stage(WaypointPullReply(s))(ctx))
    @api def ===(that: WaypointPull): Bool = ???
    @api def =!=(that: WaypointPull): Bool = ???
    @api def toText: Text                  = textify(this)
  }

  case class WaypointPullRep(s: Exp[WaypointPullRep])
      extends MetaAny[WaypointPullRep] {
    @api def success: Bool = Bool(stage(WaypointPullRep_reply_success(s))(ctx))
    @api def wp_received: FixPt[FALSE, _32, _0] =
      FixPt(stage(WaypointPullRep_reply_wp_received(s))(ctx))
    @api def ===(that: WaypointPullRep): Bool = ???
    @api def =!=(that: WaypointPullRep): Bool = ???
    @api def toText: Text                     = textify(this)
  }

  case class WaypointPullRep_reply_success(srv: Exp[WaypointPullRep])
      extends Op[Bool] {
    def mirror(f: Tx) =
      stage(WaypointPullRep_reply_success(f(srv)))(EmptyContext)
  }

  case class WaypointPullRep_reply_wp_received(srv: Exp[WaypointPullRep])
      extends Op[FixPt[FALSE, _32, _0]] {
    def mirror(f: Tx) =
      stage(WaypointPullRep_reply_wp_received(f(srv)))(EmptyContext)
  }

  case class WaypointPullReply(srv: Exp[WaypointPull])
      extends Op[WaypointPullRep] {
  ...
  object WaypointPull {
    ...
    @api def apply(): WaypointPull =
      WaypointPull(stage(NewWaypointPull())(ctx))
  }
