...
trait AltitudeApi extends AltitudeExp { self: RosApi =>

}

trait AltitudeExp { self: RosExp =>
  ...
  case class Altitude(s: Exp[Altitude]) extends MetaAny[Altitude] {
    @api def monotonic: FltPt[_24, _8] =
      FltPt(stage(Altitude_monotonic(s))(ctx))
    ...
    @api def bottom_clearance: FltPt[_24, _8] =
      FltPt(stage(Altitude_bottom_clearance(s))(ctx))
    @api def ===(that: Altitude) = ???
    @api def =!=(that: Altitude) = ???
    @api def toText: Text        = textify(this)
  }

  case class Altitude_monotonic(msg: Exp[Altitude])
  ...
  case class Altitude_bottom_clearance(msg: Exp[Altitude])
      extends Op[FltPt[_24, _8]] {
    def mirror(f: Tx) = stage(Altitude_bottom_clearance(f(msg)))(EmptyContext)
  }

  case class NewAltitude(monotonic: Exp[FltPt[_24, _8]],
                         amsl: Exp[FltPt[_24, _8]],
                         local: Exp[FltPt[_24, _8]],
                         relative: Exp[FltPt[_24, _8]],
                         terrain: Exp[FltPt[_24, _8]],
                         bottom_clearance: Exp[FltPt[_24, _8]])
      extends Op[Altitude] {
    def mirror(f: Tx) =
      stage(
        NewAltitude(f(monotonic),
                    f(amsl),
                    f(local),
                    f(relative),
                    f(terrain),
                    f(bottom_clearance)))(EmptyContext)
  }

  object Altitude {

    @api
    def apply(monotonic: FltPt[_24, _8],
              amsl: FltPt[_24, _8],
              local: FltPt[_24, _8],
              relative: FltPt[_24, _8],
              terrain: FltPt[_24, _8],
              bottom_clearance: FltPt[_24, _8]): Altitude =
      Altitude(
        stage(
          NewAltitude(monotonic.s,
                      amsl.s,
                      local.s,
                      relative.s,
                      terrain.s,
                      bottom_clearance.s))(ctx))

  }

}

trait ScalaGenAltitude extends ScalaCodegen {
