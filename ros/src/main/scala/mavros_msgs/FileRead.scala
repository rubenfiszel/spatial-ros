...
trait FileReadApi extends FileReadExp { self: RosApi =>

}

trait FileReadExp { self: RosExp =>
  ...
  case class FileRead(s: Exp[FileRead]) extends MetaAny[FileRead] {
    ...
    @api def offset: FixPt[FALSE, _64, _0] =
      FixPt(stage(FileRead_offset(s))(ctx))
  ...
  case class NewFileRead(file_path: Exp[Text],
                         offset: Exp[FixPt[FALSE, _64, _0]],
                         size: Exp[FixPt[FALSE, _64, _0]])
      extends Op[FileRead] {
    def mirror(f: Tx) =
      stage(NewFileRead(f(file_path), f(offset), f(size)))(EmptyContext)
  }
  ...
  object FileRead {
    ...
    def apply(file_path: Text,
              ...
              size: FixPt[FALSE, _64, _0]): FileRead =
      FileRead(stage(NewFileRead(file_path.s, offset.s, size.s))(ctx))
  }
