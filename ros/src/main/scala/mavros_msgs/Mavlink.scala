...
trait MavlinkApi extends MavlinkExp { self: RosApi =>

}

trait MavlinkExp { self: RosExp =>
  ...
  case class Mavlink_sysid(msg: Exp[Mavlink])
      extends Op[FixPt[FALSE, _8, _0]] {
    def mirror(f: Tx) = stage(Mavlink_sysid(f(msg)))(EmptyContext)
  }

  case class Mavlink_compid(msg: Exp[Mavlink])
      extends Op[FixPt[FALSE, _8, _0]] {
    def mirror(f: Tx) = stage(Mavlink_compid(f(msg)))(EmptyContext)
  }

  case class Mavlink_msgid(msg: Exp[Mavlink])
      extends Op[FixPt[FALSE, _8, _0]] {
    def mirror(f: Tx) = stage(Mavlink_msgid(f(msg)))(EmptyContext)
  }

  case class Mavlink_checksum(msg: Exp[Mavlink])
      extends Op[FixPt[FALSE, _16, _0]] {
    def mirror(f: Tx) = stage(Mavlink_checksum(f(msg)))(EmptyContext)
  }

  case class Mavlink_payload64(msg: Exp[Mavlink])
      extends Op[MetaArray[FixPt[FALSE, _64, _0]]] {
    def mirror(f: Tx) = stage(Mavlink_payload64(f(msg)))(EmptyContext)
  }

  case class NewMavlink(is_valid: Exp[Bool],
                        len: Exp[FixPt[FALSE, _8, _0]],
                        seq: Exp[FixPt[FALSE, _8, _0]],
                        sysid: Exp[FixPt[FALSE, _8, _0]],
                        compid: Exp[FixPt[FALSE, _8, _0]],
                        msgid: Exp[FixPt[FALSE, _8, _0]],
                        checksum: Exp[FixPt[FALSE, _16, _0]],
                        payload64: Exp[MetaArray[FixPt[FALSE, _64, _0]]])
      extends Op[Mavlink] {
    def mirror(f: Tx) =
      stage(
        NewMavlink(f(is_valid),
                   f(len),
                   f(seq),
                   f(sysid),
                   f(compid),
                   f(msgid),
                   f(checksum),
                   f(payload64)))(EmptyContext)
  }

  object Mavlink {

    @api
    def apply(is_valid: Bool,
              len: FixPt[FALSE, _8, _0],
              seq: FixPt[FALSE, _8, _0],
              sysid: FixPt[FALSE, _8, _0],
              compid: FixPt[FALSE, _8, _0],
              msgid: FixPt[FALSE, _8, _0],
              checksum: FixPt[FALSE, _16, _0],
              payload64: MetaArray[FixPt[FALSE, _64, _0]]): Mavlink =
      Mavlink(
        stage(
          NewMavlink(is_valid.s,
                     len.s,
                     seq.s,
                     sysid.s,
                     compid.s,
