...
trait WaypointExp {
  ...
  implicit object WaypointMsg extends Msg[Waypoint]
...
trait ScalaGenWaypoint extends RosScalaCodegen {
  val IR: RosExp
  import IR._

  override protected def emitNode(lhs: Sym[_], rhs: Op[_]): Unit = rhs match {
    case Waypoint_frame(msg) => emit(src"val $lhs = $msg.frame")
    case Waypoint_command(msg) => emit(src"val $lhs = $msg.command")
    case Waypoint_is_current(msg) => emit(src"val $lhs = $msg.is_current")
    case Waypoint_autocontinue(msg) => emit(src"val $lhs = $msg.autocontinue")
    case Waypoint_param1(msg) => emit(src"val $lhs = $msg.param1")
    case Waypoint_param2(msg) => emit(src"val $lhs = $msg.param2")
    case Waypoint_param3(msg) => emit(src"val $lhs = $msg.param3")
    case Waypoint_param4(msg) => emit(src"val $lhs = $msg.param4")
    case Waypoint_x_lat(msg) => emit(src"val $lhs = $msg.x_lat")
    case Waypoint_y_long(msg) => emit(src"val $lhs = $msg.y_long")
    case Waypoint_z_alt(msg) => emit(src"val $lhs = $msg.z_alt")
    case NewWaypoint(frame, command, is_current, autocontinue, param1, param2, param3, param4, x_lat, y_long, z_alt) =>
      emit(src"val $lhs = Waypoint($frame, $command, $is_current, $autocontinue, $param1, $param2, $param3, $param4, $x_lat, $y_long, $z_alt)")
