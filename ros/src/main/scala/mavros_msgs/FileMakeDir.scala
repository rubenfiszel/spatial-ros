...
trait FileMakeDirApi extends FileMakeDirExp { self: RosApi =>

}

trait FileMakeDirExp { self: RosExp =>
  ...
  case class FileMakeDir(s: Exp[FileMakeDir]) extends MetaAny[FileMakeDir] {
    ...
    @api def response: FileMakeDirRep =
      FileMakeDirRep(stage(FileMakeDirReply(s))(ctx))
    @api def ===(that: FileMakeDir): Bool = ???
    @api def =!=(that: FileMakeDir): Bool = ???
    @api def toText: Text                 = textify(this)
  }

  case class FileMakeDirRep(s: Exp[FileMakeDirRep])
      extends MetaAny[FileMakeDirRep] {
    @api def success: Bool = Bool(stage(FileMakeDirRep_reply_success(s))(ctx))
    @api def r_errno: FixPt[TRUE, _32, _0] =
      FixPt(stage(FileMakeDirRep_reply_r_errno(s))(ctx))
  ...
  case class FileMakeDirRep_reply_success(srv: Exp[FileMakeDirRep])
      extends Op[Bool] {
    def mirror(f: Tx) =
      stage(FileMakeDirRep_reply_success(f(srv)))(EmptyContext)
  }

  case class FileMakeDirRep_reply_r_errno(srv: Exp[FileMakeDirRep])
      extends Op[FixPt[TRUE, _32, _0]] {
    def mirror(f: Tx) =
      stage(FileMakeDirRep_reply_r_errno(f(srv)))(EmptyContext)
  }

  case class FileMakeDirReply(srv: Exp[FileMakeDir])
      extends Op[FileMakeDirRep] {
  ...
  object FileMakeDir {
    ...
    @api def apply(dir_path: Text): FileMakeDir =
      FileMakeDir(stage(NewFileMakeDir(dir_path.s))(ctx))
  }
