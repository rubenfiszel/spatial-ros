...
trait RCOutApi extends RCOutExp { self: RosApi =>

}

trait RCOutExp { self: RosExp =>
  ...
  case class RCOut(s: Exp[RCOut]) extends MetaAny[RCOut] {
    @api def channels: MetaArray[FixPt[FALSE, _16, _0]] =
      MetaArray(stage(RCOut_channels(s))(ctx))
    @api def ===(that: RCOut) = ???
    @api def =!=(that: RCOut) = ???
    @api def toText: Text     = textify(this)
  }

  case class RCOut_channels(msg: Exp[RCOut])
      extends Op[MetaArray[FixPt[FALSE, _16, _0]]] {
    def mirror(f: Tx) = stage(RCOut_channels(f(msg)))(EmptyContext)
  }

  case class NewRCOut(channels: Exp[MetaArray[FixPt[FALSE, _16, _0]]])
      extends Op[RCOut] {
  ...
  object RCOut {
    ...
    @api def apply(channels: MetaArray[FixPt[FALSE, _16, _0]]): RCOut =
      RCOut(stage(NewRCOut(channels.s))(ctx))
